...
export default function App() {
  ...
  const sendMessage = async (e) => {
    ...
    const warmUpTimer = setTimeout(() => {
      setLoadingMessage("Waking up the AI server on Render (takes ~50-60s on cold start)...");
    }, 5000);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev, 
        { role: "assistant", content: "The AI agent is currently waking up from sleep on Render. Please wait around 50–60 seconds and try sending your message again." }
      ]);
    } finally {
      ...
      {messages.length === 0 ? (
        ...
        <div className="chat-screen">
          ...
          <div className="messages-area">
            ...
            {loading && (
              <div className="message-bubble ai typing-bubble" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <span className="dot"></span>
                <span className="dot"></span>
                <span className="dot"></span>
                <span style={{ fontSize: "12px", opacity: 0.85, marginLeft: "6px" }}>{loadingMessage}</span>
              </div>
            )}
